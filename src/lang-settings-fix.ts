...
import { ResolveCommandRegistry, type ResolveCommandHook } from './app_api/index';
...
const hook: ResolveCommandHook = function (resolveCommand, payload, extra) {
  const passthrough = () => {
    console.warn(`[lang-settings-fix] Passing through due to payload mismatch."`);
    return resolveCommand(payload, extra);
  };

  if (!payload.setClientSettingEndpoint || typeof payload.setClientSettingEndpoint !== 'object')
    return passthrough();

  const setClientSettingEndpoint = payload.setClientSettingEndpoint as Record<string, unknown>;

  if (!Array.isArray(setClientSettingEndpoint.settingDatas)) return passthrough();
