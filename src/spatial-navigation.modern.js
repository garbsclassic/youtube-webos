...
(function () {
  ...
  function spatialNavigationHandler() {
    window.addEventListener('keydown', e => {
      const currentKeyMode =
        window.parent?.__spatialNavigation__?.keyMode ?? window.__spatialNavigation__?.keyMode;
  ...
  function navigate(dir) {
    ...
    if (startingPoint) {
      elementFromPosition =
        document.elementFromPoint(startingPoint.x, startingPoint.y) ?? document.body;
      ...
      if (
        ...
        if (action === 'scroll' && scrollingController(eventTarget, dir)) return;
          ...
          bestInsideCandidate = eventTarget.spatialNavigationSearch(dir, {
            ...
            candidates: getSpatialNavigationCandidates(eventTarget, { mode: 'all' })
          });
  ...
  function focusingController(bestCandidate, dir) {
    if (bestCandidate) {
      ...
      bestCandidate.focus({
        preventScroll: container === window || getCSSSpatNavAction(container) !== 'focus'
      });
      startingPoint = null;
      return true;
    }
    ...
    if (bestTarget && isDelegableContainer(bestTarget)) {
      ...
      if (descendantsBest) bestTarget = descendantsBest;
        ...
        bestTarget = candidates.length
          ? targetElement.spatialNavigationSearch(dir, { candidates, container })
          : null;
      }
    }
    return bestTarget;
  ...
  function filteredCandidates(currentElm, candidates, dir, container) {
    ...
    const eventTargetRect =
      originalContainer.parentElement && container !== originalContainer && !isVisible(currentElm)
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      if (isCurrentContainer) {
        if (
          (currentElm.contains(candidate) && isInside(eventTargetRect, candidateRect)) ||
        ...
        const candidateBody =
          candidate.nodeName === 'IFRAME' ? candidate.contentDocument.body : null;
        if (
  ...
  function selectBestCandidate(currentElm, candidates, dir) {
    ...
    const isGrid =
      getCachedComputedStyle(container).getPropertyValue('--spatial-navigation-function').trim() ===
      'grid';
    ...
    if (isGrid) {
      const aligned = [];
      for (let i = 0; i < candidates.length; i++) {
        if (isAligned(currentTargetRect, getBoundingClientRect(candidates[i]), dir))
  ...
  function getScrollContainer(element) {
    ...
    return scrollContainer === document || scrollContainer === document.documentElement
      ? window
      : scrollContainer;
  }
  ...
  function isHTMLScrollBoundary(element, dir) {
    switch (dir) {
      ...
      case 'right':
        return Math.abs(element.scrollWidth - element.scrollLeft - element.clientWidth) <= 1;
      case 'up':
  ...
  function isVisibleInScroller(element) {
    ...
    return (
      isInside(scrollerRect, elementRect, 'left') && isInside(scrollerRect, elementRect, 'down')
    );
  }
  ...
  function isAtagWithoutHref(element) {
    return (
      element.tagName === 'A' && !element.hasAttribute('href') && !element.hasAttribute('tabIndex')
    );
  }
  ...
  function isInside(containerRect, childRect) {
    return (
      ((containerRect.left <= childRect.right && containerRect.right >= childRect.right) ||
        (containerRect.left <= childRect.left && containerRect.right >= childRect.left)) &&
      ((containerRect.top <= childRect.top && containerRect.bottom >= childRect.top) ||
        (containerRect.top <= childRect.bottom && containerRect.bottom >= childRect.bottom))
    );
  }
  ...
  function getDistance(searchOrigin, candidateRect, dir) {
    ...
    if (dir) {
      const alignBias = isAligned(searchOrigin, candidateRect, dir)
        ? Math.min(
            intersectionRect[isLR ? 'height' : 'width'] / searchOrigin[isLR ? 'height' : 'width'],
            1
          )
        : 0;
  ...
  function getOverlappedCandidates(targetElement) {
    ...
    for (let i = 0; i < areas.length; i++) {
      if (targetElement !== areas[i] && isEntirelyVisible(areas[i], targetElement))
        result.push(areas[i]);
    }
    return result;
  }
